...
contract StakingDapp is Ownable, ReentrancyGuard {
   ...
   using SafeERC20  for IERC20;  // math library
   ...
   function withdraw(uint _pid, uint _amount)public nonReentrant()  nonReentrant() {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage  user = userInfo[_pid][msg.sender];
  ...
  function claimReward(uint _pid)public nonReentrant{
    ...
    function _createNotification(uint _id, uint amount, address _user, string memory _typeOf)internal{
